...
class EditPatientContactTracingScreen extends StatefulWidget {
  static const routeName = '/patient_contact_tracing_edit';

  @override
  _EditPatientContactTracingScreenState createState() =>
      _EditPatientContactTracingScreenState();
}

class _EditPatientContactTracingScreenState
    extends State<EditPatientContactTracingScreen> {
...
initState(){
  newContact = Provider.of<Patients>(context,listen:true).selectedPatient.tracingDetail;
}


  submitTrackingAndPop() async {
    print("submitTrackingAndMoveToHome called");

    //print(json.encode(screeningResult));

    await Provider.of<Patients>(context, listen: false)
        .editContactTracking(newContact);
  }
